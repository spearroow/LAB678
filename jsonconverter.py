...
class JSONConverter:
    ...
    def convert_file(input_path, output_path):
        if not FileVerifier.verify_json(input_path):
            return False
        ...
        try:
            ...
            with open(output_path, 'w') as file:
                if output_type == '.json':
                    file.write(JSONConverter.convert_to_json(data))
                elif output_type in ['.yml', '.yaml']:
                    file.write(JSONConverter.json_to_yaml(data))
                elif output_type == '.xml':
                    file.write(JSONConverter.json_to_xml(data))
                else:
